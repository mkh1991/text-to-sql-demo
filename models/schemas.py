...
class CellData(BaseModel):
    ...
    sql_response: Optional[SQLQuery] = Field(
        default=None, description="Generated SQL query"
    )
    execution_result: Optional[Dict[str, Any]] = Field(
        default=None, description="Query execution results"
    )
    analysis_result: Optional[QueryonData] = Field(
        default=None, description="Data analysis results"
    )
    status: str = Field(default="empty", description="Cell execution status")
    temp_table_name: Optional[str] = Field(
        default=None, description="Associated temporary table name"
    )
    created_at: Optional[str] = Field(
        default=None, description="Cell creation timestamp"
    )
...
class DatabaseTable(BaseModel):
    ...
    source_cell_id: Optional[int] = Field(
        default=None, description="Cell ID if temp table"
    )
...
class ExecutionResult(BaseModel):
    ...
    data: Optional[Any] = Field(
        default=None, description="Result data (usually DataFrame)"
    )
...
class LLMConfig(BaseModel):
    ...
    model: str = Field(
        default="google/gemini-2.5-flash", description="Model identifier"
    )
